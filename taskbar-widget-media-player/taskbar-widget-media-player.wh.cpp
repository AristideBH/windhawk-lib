...
> **Fork:** this is a fork of [Salyts' Taskbar Fluent Media
> Player](https://github.com/Salyts/Taskbar-Fluent-Media-Player) (MIT
> licensed - see this folder's `LICENSE`), modified only to register as a
> widget in [`taskbar-widget-stack`](../taskbar-widget-stack/README.md)'s
> pane when that mod is installed and enabled, alongside
> [`taskbar-widget-system-usage`](../taskbar-widget-system-usage/README.md).
> Every feature below is the original mod's own, unmodified - this fork adds
> a cross-mod registration path and a fallback to the original mod's own
> standalone injection when the host isn't present, nothing else.
...
// ---------------------------------------------------------------------
// Cross-mod widget ABI (fork-only addition - see this file's top-of-file
// note and ../taskbar-widget-stack/PLAN.md's Incident 35/38 for the full
// design). Struct/typedef layout duplicated byte-for-byte from
// taskbar-widget-stack.wh.cpp's own copy - there's no shared header
// between separately-built Windhawk mods, so this has to be kept in
// sync by hand between the two files (matches
// taskbar-widget-system-usage.wh.cpp's own copy of the same block).
// ---------------------------------------------------------------------

constexpr wchar_t kMPRegisterWidgetPropName[] =
    L"TaskbarWidgetStack_RegisterWidgetFn_v1";
constexpr wchar_t kMPUnregisterWidgetPropName[] =
...
// Registration state - mirrors taskbar-widget-system-usage.wh.cpp's own
// g_remoteRegistered/g_hostRegisterFn/etc. exactly, just prefixed `mp`
// to avoid any confusion with this file's own many globals.
bool g_mpRemoteRegistered = false;
...
// host->parentPanelAbi is unwrapped via winrt::copy_from_abi (an owned
// copy, since this needs to outlive the call - see taskbar-widget-
// stack.wh.cpp's RemoteWidget::Create for the send-side winrt::get_abi
// this pairs with). Reuses BuildPlayerGrid() verbatim - the original
// mod's own content-building code is untouched; only where the result
// attaches (host->parentPanelAbi vs. whatever ResolveInjectionTarget
// would have picked) and how big a slot it's given (host->paneHeight,
// via a wrapping Border, matching every other widget in the stack -
// see taskbar-widget-stack.wh.cpp's PlaceholderWidget::Create) differ
// from the original InjectPlayerGrid.
double __cdecl MediaPlayer_Create(void* /*context*/,
    ...
    try {
        ...
        g_mpRemoteParentPanel = parent;
        g_mpRemoteContainer = container;
        ...
        Wh_Log(L"MediaPlayer_Create: built player as a registered widget");
        return desiredWidth;
    } catch (...) {
        Wh_Log(L"MediaPlayer_Create: exception");
        return 0.0;
    }
}

void __cdecl MediaPlayer_Tick(void* /*context*/) {
    // No-op: this mod already drives its own refresh cadence via its
    // own timer/media-session threads (StartTimerThread/StartMediaThread),
    // independent of the host's widget stack - it doesn't need the
    // host's shared tick signal, same as taskbar-widget-system-usage.
}

double __cdecl MediaPlayer_OnSettingsChanged(void* /*context*/) {
    // The host's IWidget contract documents Destroy()-then-Create() as
    // the mechanism it actually uses on a settings change, so nothing
    // calls this directly today (Wh_ModSettingsChanged below follows
    // that same unregister-then-reregister pattern); kept only because
    // the ABI struct needs a typed slot here.
    return g_playerGrid ? g_playerGrid.ActualWidth() : 0.0;
}

// Deliberately does NOT call RemovePlayerGrid() - that function assumes
// g_injectionParent is a Grid with a real ColumnDefinition it owns
// (tray-column or edge-position placement), which isn't true here
// (g_mpRemoteParentPanel is the host's own widgetsPanel, a StackPanel).
// Duplicates the handful of RemovePlayerGrid's own cleanup lines that
// ARE universally safe (closing an open mini player flyout, clearing
// cached album art/title state) rather than editing that function to
// special-case a StackPanel target - keeps RemovePlayerGrid, and every
// standalone code path that calls it, completely untouched.
void __cdecl MediaPlayer_Destroy(void* /*context*/) {
    try {
        if (g_miniPlayerFlyoutOpen && g_miniPlayerFlyout) {
            g_miniPlayerExplicitCloseRequested.store(true);
            try {
    ...
    try {
        if (g_mpRemoteParentPanel && g_mpRemoteContainer) {
            uint32_t index;
            if (g_mpRemoteParentPanel.Children().IndexOf(g_mpRemoteContainer,
                                                           index)) {
                g_mpRemoteParentPanel.Children().RemoveAt(index);
            }
        }
    } catch (...) {
        Wh_Log(L"MediaPlayer_Destroy: exception");
    }
    g_playerGrid = nullptr;
    g_mpRemoteParentPanel = nullptr;
    g_mpRemoteContainer = nullptr;
...
void __cdecl MediaPlayer_GetId(void* /*context*/,
                                wchar_t* buffer,
                                int bufferSize) {
    wcsncpy_s(buffer, bufferSize, L"taskbar-widget-media-player", _TRUNCATE);
}

void __cdecl MediaPlayer_GetDisplayName(void* /*context*/,
                                         wchar_t* buffer,
                                         int bufferSize) {
    wcsncpy_s(buffer, bufferSize, L"Media Player", _TRUNCATE);
}

void FillMPWidgetAbi(WidgetStackWidgetAbiV1& abi) {
    abi = {};
    abi.context = kMPWidgetContext;
    abi.Create = &MediaPlayer_Create;
    abi.Tick = &MediaPlayer_Tick;
    abi.OnSettingsChanged = &MediaPlayer_OnSettingsChanged;
    abi.Destroy = &MediaPlayer_Destroy;
    abi.GetId = &MediaPlayer_GetId;
    abi.GetDisplayName = &MediaPlayer_GetDisplayName;
}

// Tries taskbar-widget-stack's registration (GetPropW discovery on
// Shell_TrayWnd) on every call, even if this mod is already active
// standalone - matches taskbar-widget-system-usage.wh.cpp's own
// TryRegisterOrInject/Incident 3: both mods' retry/readiness paths run
// independently with no ordering guarantee between them, so the host's
// property frequently isn't published yet the first time this runs.
// Falls back to (or stays on) this mod's own original ApplySettings()
// standalone injection when the host still isn't found. Must run on the
// taskbar's own UI thread, same as every existing ApplySettings() call
// site this replaces/wraps.
void TryRegisterOrApplySettings(HWND hWnd) {
    ...
    auto registerFn = (WidgetStack_RegisterWidget_t)GetPropW(
        hWnd, kMPRegisterWidgetPropName);
    if (registerFn) {
        if (g_playerGrid) {
            // Already active standalone - tear that down first so the
            // two don't end up duplicated on the taskbar.
            RemovePlayerGrid();
        }
        ...
        if (registerFn(&abi)) {
            ...
            g_mpHostUnregisterFn = (WidgetStack_UnregisterWidget_t)GetPropW(
                hWnd, kMPUnregisterWidgetPropName);
...
static void WINAPI TrayUI_StartTaskbar_Hook(void* pThis) {
    ...
    g_trackPosition   = L"";
    g_layoutUpdateToken = {};
    // The old XamlRoot content (including whatever taskbar-widget-stack
    // pane we may have been registered into) is dying either way, same
    // reasoning as the resets just above - if the host's own hook
    // already ran its Destroy() call on us this cycle, these are
    // already null; if it hasn't (yet, or at all), there's nothing
    // meaningful left to detach from.
    g_mpRemoteParentPanel = nullptr;
    g_mpRemoteContainer = nullptr;
...
void Wh_ModUninit() {
    ...
    if (g_taskbarWnd)
        RunFromWindowThread(g_taskbarWnd, [](void*) {
            if (g_mpRemoteRegistered && g_mpHostUnregisterFn) {
                // Calls back into MediaPlayer_Destroy (which also does
...
void Wh_ModSettingsChanged() {
    ...
    if (hWnd) {
        ...
        bool ok = RunFromWindowThread(hWnd, [](void*) {
            try {
                if (g_mpRemoteRegistered && g_mpHostUnregisterFn &&
                    g_mpHostRegisterFn && g_mpRemoteHostHwnd) {
                    // Same Destroy()-then-Create() pattern as the
                    // standalone branch below, routed through the
                    // host's (un)registration functions instead of
